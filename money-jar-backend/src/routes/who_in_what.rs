...
pub fn who_in_what_routes() -> Router<AppState> {
    Router::new()
        .route("/CreateWhoInWhat", post(post_create_who_in_what))
        .route("/GetUsersFromEvent", post(post_get_users_from_event))
        .route("/GetEventsFromUser", post(post_get_events_from_user))
        .route("/DeleteWhoInWhat", post(post_delete_who_in_what))
}
...
async fn post_get_users_from_event(
    ...
    match response {
        Err(_) => (
            ...
            Json(GetUsersFromEventResponse { users: vec![] }),
        ),
...
async fn post_get_events_from_user(
    ...
    match response {
        Err(_) => (
            StatusCode::NOT_FOUND,
            Json(GetEventsFromUserResponse { events: vec![] }),
        ),
