...
pub fn create_event(
    ...
    event_name: String,
    event_reoccuring: bool,
) -> Result<String, Error> {
    let event_id = Uuid::new_v4().to_string();
    let event = NewEvent::new(
        event_id.clone(),
        event_owner_id.clone(),
        event_name,
        event_reoccuring,
    );

    // Create the event
    let result = insert_into(Events).values(event).execute(conn);
    ...
    match result {
        ...
        Err(e) => {
            ...
            return Err(Error::DatabaseError(
                ...
                Box::new("Failed to create event".to_string()),
            ));
        }
    }
    ...
    let result = insert_into(WhoInWhat).values(who_in_what).execute(conn);
    ...
    match result {
        ...
        Err(e) => {
            ...
            return Err(Error::DatabaseError(
                diesel::result::DatabaseErrorKind::UniqueViolation,
                Box::new("Failed to add owner to event".to_string()),
            ));
        }
    }
...
pub fn update_reoccuring(
    conn: &mut SqliteConnection,
    event_id: String,
    user_reoccuring: bool,
) -> Result<(), Error> {
    update(Events)
        .filter(id.eq(event_id))
...
pub fn update_reoccuring_interval(
    conn: &mut SqliteConnection,
    event_id: String,
    user_reoccuring_interval: String,
) -> Result<(), Error> {
    update(Events)
        .filter(id.eq(event_id))
...
pub fn update_final_date(
    conn: &mut SqliteConnection,
    event_id: String,
    user_final_date: String,
) -> Result<(), Error> {
    update(Events)
        .filter(id.eq(event_id))
...
pub fn get_all_events(
    conn: &mut SqliteConnection,
    user_owner_id: String,
) -> Result<Vec<GetEvent>, Error> {
    println!("Searching for events with owner_id: {}", user_owner_id);
    let events: Vec<GetEvent> = Events
        .filter(owner_id.eq(user_owner_id.clone()))
        .load::<GetEvent>(conn)
        .map_err(|e| {
    ...
    if events.is_empty() {
        ...
        for event in &events {
            println!(
                "Found event: id={}, name={}, owner_id={}",
                event.id, event.name, event.owner_id
            );
        }
    }
    Ok(events)
