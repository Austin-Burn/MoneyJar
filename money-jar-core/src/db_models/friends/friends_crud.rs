...
pub fn create_friend(
    ...
    let friend_exists = select(exists(
        Friends.filter(
            user_id
                .eq(friend_one.clone())
                .and(friend_id.eq(friend_two.clone())),
        ),
    ))
    ...
    .map_err(|e| {
        diesel::result::Error::DatabaseError(
            ...
            Box::new(format!("Failed to check friendship: {}", e)),
        )
    })?;
    ...
    insert_into(Friends)
        ...
        .map_err(|e| {
            diesel::result::Error::DatabaseError(
                DatabaseErrorKind::Unknown,
                Box::new(format!("Failed to create friendship: {}", e)),
            )
        })?;
    ...
    insert_into(Friends)
        ...
        .map_err(|e| {
            diesel::result::Error::DatabaseError(
                DatabaseErrorKind::Unknown,
                Box::new(format!("Failed to create reverse friendship: {}", e)),
            )
        })?;
...
pub fn get_friends(conn: &mut SqliteConnection, search_id: String) -> Result<Vec<String>, Error> {
    let friends: Vec<String> = Friends
        ...
        .map_err(|e| {
            Error::DatabaseError(
                DatabaseErrorKind::Unknown,
                Box::new(format!("Failed to get friends: {}", e)),
            )
        })?;
    Ok(friends)
}
...
pub fn delete_friend(
    ...
    delete(Friends.filter(user_id.eq(search_id).and(friend_id.eq(id_two))))
        ...
        .map_err(|e| {
            diesel::result::Error::DatabaseError(
                DatabaseErrorKind::Unknown,
                Box::new(format!("Failed to delete friendship: {}", e)),
            )
        })?;
    ...
    delete(Friends.filter(user_id.eq(&id_two).and(friend_id.eq(&search_id))))
        ...
        .map_err(|e| {
            diesel::result::Error::DatabaseError(
                DatabaseErrorKind::Unknown,
                Box::new(format!("Failed to delete reverse friendship: {}", e)),
            )
        })?;
